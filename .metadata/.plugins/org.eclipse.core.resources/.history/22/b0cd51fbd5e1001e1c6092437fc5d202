...
public class MainController {
 @Autowired
 private NodeService nodeService; // DB 연동을 위해 NodeService @AutoWired를 통해 자동 주입.
 ...
 public JsonResult getPoi(@RequestParam double x, @RequestParam double y) throws IOException, InterruptedException {
   ...
   List<Point> pointByKeyword = KakaoApiUtil.getPointByKeyword("약국", center); // 중심좌표를 기준으로 약국 ㅁ목록 카카오 API로 조회하기
   List<Node> nodeList = new ArrayList<>();
   for (Point point : pointByKeyword) {
     Node node = nodeService.getOne(Long.valueOf(point.getId())); //조회한 약국정보가 DB에 저장 되어 있는지 조회
     if (node == null) {
       node = new Node();
       ...
       nodeService.add(node); // 저장 되어 있지 않으면 저장.
     }
     nodeList.add(node);
   }
